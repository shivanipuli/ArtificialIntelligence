...
def iterative(state, conflicts):
    if sum(conflicts)==0:
        return state
    fringe=[(conflicts,state)]
    heapify(fringe)
    while len(fringe)>0:
        tempConf,tempState=heappop(fringe)
        if tempConf==0:
            return tempState
        row = get_next_unassigned_var(tempState, tempConf)
        for new_conflict, col in get_sorted_values(tempState, row):
            new_state = state[:row] + [col] + state[row + 1:]
            new_conflict=[collisions(new_state, i) for i in range(len(new_state))]
            heappush(fringe,(new_state,new_conflict))
    return None
...
def solvePuzzle(size):
    ...
    return iterative(board,conflict)
...
#     print("Size: " + str(i))
#     #tempT = time.perf_counter()
#     flawedBoard=None
#     if i in []:#[10,11,12,19,25,42]:
#         flawedBoard=nQueens(i)
#     else:
#         flawedBoard=solvePuzzle(i)
#     print(flawedBoard)
#     #print("Check  Solution: " + str(test_solution(flawedBoard)))
#     #print("Time: %ss" %(time.perf_counter()-tempT))
#     print()
